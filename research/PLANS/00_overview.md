...
成熟模块在仓库根的 **`pwsf/` 包**里，取证脚本留在 `TOOLS/`。
命令用法见根目录 [`README.md`](../README.md)。

| 模块 | 作用 |
|---|---|
| `pwsf.config` | 路径与常量，支持 `PWSF_*` 环境变量 / `pwsf.local.json` 覆盖 |
| `pwsf.crypto` | `name_hash` / `MT19937`(自定义 LCG 播种) / `buffer_xor_decrypt` |
| `pwsf.olang` | RBX 容器解析（三级索引 + 字符串池） |
| `pwsf.olang_build` | olang 序列化器（池重排 + 去重，往返字节一致） |
| `pwsf.archive` | PDT/DAT 归档解析 + payload 解密 + CRC-32 校验 |
| `pwsf.archive_index` | 全盘容器索引 → `ANALYSIS/_archive_index.tsv` |
| `pwsf.names` | `entry_name_hash` / `str_hash24` / 67 项扩展名表 / 哈希反演 |
| `pwsf.briefing` | BRIEFING/CODEC：逐扇区解密 + 记录头解析 + 台词导出 |
| `pwsf.subtitle` | 游戏内字幕：8 个 olang group → `ANALYSIS/subtitle_ingame.tsv` |
| `pwsf.xpr` | XPR2 容器解包 / 重打包（大端，往返字节一致） |
