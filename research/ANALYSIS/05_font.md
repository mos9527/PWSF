...
1. `xpr_package_load` @ `0x140042630` 在整个映像里**只有一个调用点**
   ——`font_load_xpr` @ `0x140042DB0`（⚠ 旧构建，且与 §3/§13.3 记的
   `0x140042C60` 互相矛盾；现行构建实测 RVA `0x42E20`，见 §13.8）。
   除字体外没有任何模块读 XPR2。
...
`font_load_xpr` @ `0x140042C60`（旧构建；现行 RVA `0x42E20`，见 §13.8）按名字取 `FontTexture` / `FontData` 两个资源，
...
`font_load_xpr` @ `0x140042C60`（旧构建；现行 RVA `0x42E20`，见 §13.8）对 `FontTexture`（TX2D，52 B）做的唯一一件事是
把指针存起来：
...
### 13.8 二进制漂移：exe 已被 Steam 更新，绝对地址全部作废（2026-09-22）

反分析时记下的映像哈希与 2026-09-22 装在游戏里的**不是同一个文件**：

| | md5 | sha256 |
|---|---|---|
| 分析时的二进制 | `5bfe6b2cdbb77c3f0f3cff05fcad22bd` | `5bc5756166e611d7…588fff` |
| 现行（Steam 更新后） | `b2a4d3ae03b8eb02f0bc9fcc8b47d8a6` | `e9c2e7241438ca70…894305` |

后果：`font_load_xpr` 从 RVA `0x42C60` 挪到了 **`0x42E20`**（+0x1C0），本文里
所有 `0x1400…` 绝对地址对现行构建都不再成立。§1 记的 `0x140042DB0` 与
§3/§13.3 的 `0x140042C60` 本来也互相矛盾（差 0x150），至少一个是笔误。

**所以路线 A 的注入 DLL 用 sigscan，不写死地址**：`hooklib64/dllmain.cpp` 拿函数
开头 28 字节（8 个寄存器 push + `lea rbp` 通配 + `sub rsp,0C8h`）整模块扫描，
2026-09-22 实测命中 `base+0x42E20`，且大/小两次字体加载（`0007ccd8.xpr`
4096×4096、`000ebbe8.xpr` 2048×1024）都从 hook 里过 —— 确认命中的就是
`font_load_xpr`。exe 再更新，只要这段 prologue 不变就不用改代码。

顺带一条：sigscan 的 hint 必须在**运行时**加上模块基址。写死 `0x140042E20`
