...
状态：**格式已打通并全量验证**。工具 `pwsf.archive`（解析）
+ `pwsf.names`（条目名哈希）+ `pwsf.archive_index`（全盘索引）。
...
**67 项**，已完整导出到 `pwsf.names` 的 `EXT_IDS`。
...
```bash
python -m pwsf.archive_index          # 全盘索引 -> research/ANALYSIS/_archive_index.tsv
python -m pwsf.archive_index -v       # 带耗时日志
python -m pwsf.archive_index --full   # 不做 CRC 字节预算，全量校验（慢）
```
