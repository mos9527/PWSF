...
- [ ] **尾部 NUL 填充是否被接受**。串是按「偏移表 + NUL 结尾」取的，最后
      一串之后的 NUL 只是残留空间，理论上无害；但必须**实机验证**。
- [ ] **实机验证**本身。这是唯一能闭合 9.3 的手段：装一份改过的
      `0076531d.DAT`，进 CODEC 通话看台词、听语音对不对得上。
      备份用 `pwsf.install` 的 `.orig` 机制，并提供 `--restore`。
