...
1. ~~**写回**~~ **已实现**：`pwsf/slotdat_build.py`，已接进 `po_import`
   （`by_slot()` → `rebuild()` → `verify()`）。选路依据：

   | 方案 | 结果 |
   |---|---|
   | 就地（池长度不变，尾部补零） | **29/43 张过场表放不下**，最坏 +1,462 B |
   | 重排池 + 重压 + 重建容器 | 34 个记录里只有 9 个 `B` 变化，总账 **A +4 / B −9 扇区** |

   就地不行是硬约束：池长度由**下一个条目的 off** 决定，而池已经装到
   99.3%（`_probe_slot27.py`）。中文按字节算并不更省——用现成的**日文副本**
   代跑（同为 CJK，3 字节/字），`ja/en` 字节比 **中位数 1.125**、均值 1.687
   （`_probe_slot28.py`）。

   实现要点：
   * 条目 off 是 u32，可以改 ⇒ 解压后的槽**重排即可，不是固定映像**
   * 没碰到的记录**密文原样搬** —— 两层 XOR 只依赖块内偏移，挪位置不改变密文
   * `extra` 块（`end-start-B` 扇区，第二个独立 zlib 流）原样搬，只挪位置
   * 新 `A`/`B` 回写 `SLOT.KEY`；12 位字段照旧截断（`A > 4095` 那两条保持原样）
   * KEY 的头 12 字节**不动**：它是 LCG 的派生种子，动了就再也解不开

   端到端（`_poc_slotdat_writeback.py`，8 条中文写进 table `0x003af54d`）：

   ```
   1/2137 records repacked, 8 string(s) written
   544,555,008 bytes -> +0
   OK: every record inflates to its declared size and all 8 translations
   read back verbatim
   ```

   `po_import` 走完整流程时还会把 2,137 条记录再全读一遍做 `verify()` ——
   慢（几分钟），但这是 544 MB 容器该有的校验强度。实测结论是
   **"其余槽位与原文件逐字节一致"**。
2. **`FEL\x07` 块**：过场记录里还有 181 个（§7.3），条目表是 8/12 字节交替，
   疑似分镜时间轴/字幕排版。解开它才能回答"哪条串进气泡、哪条走底部字幕"。

   > **2026-09-21 进展（见 [09 §10](09_stagedat_payload.md#10-felx07-是什么2026-09-21)）**：
   > 结构是 `u16 opcode / u16 size / u32 操作数`（op 3 带 1 个、op 4 带 2 个），
   > **确认是命令流**；181 个块全部扫过，**没有任何 ≥8 字符的 ASCII 串** ——
   > 所以字符串不在 FEL 里内联，FEL 只可能是指向 `RBX` 池的索引。
   > 气泡/底部字幕的分工仍待解，但不影响过场语料的完整性。
3. ~~**`GTT\x00` 池**（462 个）：另一种文本容器，没解。~~
   **已解，而且是一整块丢失的语料**：见
   [11_gtt_text.md](11_gtt_text.md) —— 462 个池在记录 0..365，池里是任务内
   无线台/提示台词，去重后 **11,118 条英文**，与 BRIEFING/SLOT-RBX/STAGEDAT
   的重合几乎为 0。那句找不到的 Miller 台词就在里面。
   （旧记录的「全在记录 0..870 那半边」也对：GTT 池落在 0..365。）
