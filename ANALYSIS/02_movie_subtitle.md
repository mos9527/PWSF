...
已由 `TOOLS/pwsf_subtitle.py` 导出，见 §6.2。
...
python pwsf_subtitle.py  # 游戏内字幕 → ..\ANALYSIS\subtitle_ingame.tsv
```
...
判据：`TOOLS/pwsf_archive_index.py` 的 `KNOWN` 集合**已包含 `"SUBTITLE"`**，
...
导出：`TOOLS/pwsf_subtitle.py` → `ANALYSIS/subtitle_ingame.tsv`，
