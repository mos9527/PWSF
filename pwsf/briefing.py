...
.. warning::
   **两套都不足以走完整段字节码**，本模块的 :func:`_decode` 是两者的**混合
   启发式**，只用于**提取**，不能用来重新发射字节码。实证见
   ``_probe_bri50.py``：

   * ``briefing_insn_decode`` 的 ``op & 0xF0`` 跳转表（``jpt_1400A4D7C``）
     只有 0x10/0x20/0x30/0x40/0x50/0x80/0x90 七个 case，**0x60/0x70 落 default，
     返回入参指针（不前进）**。故纯用它走顶层流：2049/2049 条记录全部卡死。
   * 顶层流由解释器 ``briefing_script_run`` 用 ``briefing_insn_operand``
     推进，只认 ``op & 0xF0 ∈ {0x00(终止), 0x30, 0x60, 0x70}``——反过来
     **不认** 0x8d/0x8e。

   混合规则仍然会产生失步噪声（0x07 字面量里有 ~54 条不是语音 ID 形态）。
   好在**回写不需要碰字节码**，见 ``ANALYSIS/03_codec.md`` §9。
...
def _decode(data, i, end):
    ...
    | ``op & 0xF0 == 0x90`` | 1 字节 |
    | ``op & 0xF0 in {0x10,0x20}`` | **本函数按 1 字节保守处理**（真实规则见下） |
    | ``op & 0xF0 == 0`` | 低操作码：0 终止；1/14 -> 3；2/3/4 -> 2；6/8 -> 4；**7 -> 2 + u8 长度**；9/10/13 -> 5；15 -> 9 |

    0x10/0x20 的真实规则（`sub_1400A3AB0` @ ``0x1400A3AB0``，由
    ``jpt_1400A4D7C`` 的 cases 16,32 进入）**已查明**：
    ...
    *a2 = (v4 >> 24) & 0xF;      /* 操作数类型 */
    ...
    if ((v4 >> 24) & 0xF0 == 0x20)          /* 即 a1[3] & 0xF0 == 0x20 */
        v8 = insn_decode(insn_decode(a1+4)); /* 两条嵌套操作数 */
    sub_1400A4210(handler, v4, v12, a3);
    return v8;
    ```

    ``v5 = u16 @ (a1+2)`` 只用于查处理函数表（``v4 & 0xF00000`` 为 0x800000 /
    0x100000 / 其它时分别取 ``off_140EA4860`` / ``unk_14117A530`` /
    ``off_140EA4880``），**与长度无关**。

    **但没有照着改**，因为改了并不更准：``_probe_bri52.py`` 的分层实验表明
    0x1x/0x2x 的长度**取决于它落在块体还是实参区**（块体走解释器规则
    ``1 + (op & 0xF)``，实参区才是上面的 4 字节规则），而本函数的混合启发式
    不区分二者。实测按 4 字节规则改：含 0x6d 的记录 841 -> 833（新解出 61 条 /
    795 行，丢掉 69 条 / 1106 行），噪声 62 -> 54 —— 大体是个平局，不值得为它
    翻动已导出的语料。保留 1 字节处理，直到分层解码器落地（PLANS/03 §2）。
    """
    op = data[i]
    if op & 0xC0 == 0xC0:
        return i + 1, i + 1, 0
