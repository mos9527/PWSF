...
LANG = {0x0D0E: "en", 0x0D32: "fr", 0x0D45: "de", 0x0D94: "it",
        0x0DB0: "ja", 0x0ED0: "es"}
EN = 0x0D0E
REF_LANGS = ("fr", "de", "it", "es")
...
def collect_olang() -> list:
    """One record per English slot, with the other languages as reference."""
    out = []
...
def collect_codec() -> list:
    rows = (ANALYSIS / "_briefing_lines.tsv").read_text(
...
def main() -> None:
    ...
    ap.add_argument("--outdir", type=Path, default=ROOT / "PO")
    ap.add_argument("--chunk", type=int, default=400,
                    help="entries per .po file (default 400)")
    ap.add_argument("--no-merge", action="store_true",
                    help="one entry per slot instead of merging by msgid")
    args = ap.parse_args()

    do_merge = not args.no_merge
    corpora = [("olang", collect_olang()), ("codec", collect_codec())]
